...
;; get first 1000 triangle numbers
...
(defn tri-iter
  ...
  (cond (<= number-of-factors (count (get-factors (reduce + (range 1 (+ 1 current))))))
          current
        :else
          (recur (+ current 1) number-of-factors)))

(defn many-tri
  [number-of-factors]
  (tri-iter number-of-factors number-of-factors))
